/*=================================================
  ...
  Placa, Motorista e Empresa, calculando:
...
function carregarFretes() {
    // Garante que db.taxasFrete existe
...
/* ── Renderiza o painel de configuração de taxas ── */
...
function calcularEExibirFretes() {
    ...
    lancamentosMes.forEach(l => {
        ...
        if (!porPlaca[placa])         porPlaca[placa]         = { nome: placa,     viagens: 0, litros: 0, frete: 0, detalhes: {} };
        ...
        l.itens.forEach(item => {
            const litros = item.qtd || 0;   // qtd carga
    ...
    // Ordena por frete desc
    const sortDesc = obj => Object.values(obj).sort((a, b) => b.frete - a.frete);
...
function renderFreteResumo() {
    ...
    if (dadosFretesAtual.totalNotas === 0) {
        ...
        resumo.innerHTML = `
            ...
            &nbsp;|&nbsp; 🛢️ Litros: <strong>${fmtL(dadosFretesAtual.totalLitros)}</strong>
            &nbsp;|&nbsp; 🚛 Frete total: <strong>${fmtR(dadosFretesAtual.totalFrete)}</strong>
        `;
    }
}

/* ── Função auxiliar: gera linhas de detalhe por combustível ── */
...
function exportarFretesExcel() {
    ...
    linhas.push(["Placa", "Viagens", "Litros (L)", "Taxa (R$/L)", "Frete (R$)"]);
    d.porPlaca.forEach(p => {
        linhas.push([p.nome, p.viagens, p.litros.toFixed(3), "", p.frete.toFixed(2)]);
...
function exportarFretesPDF() {
    ...
    const secoes = [
        { titulo: "Por Placa",      dados: d.porPlaca },
        { titulo: "Por Motorista",  dados: d.porMotorista },
        { titulo: "Por Empresa",    dados: d.porEmpresa }
    ];
